...
function cartas(lista, game) {
  ...
  function addCard(card, index, array) {
    ...
    childs.push(builder.element('img', { src: path + game + '/cards/' + cardClass + '/' + card.cardnumber + '.png', draggable: "false", height: '100%', width: '100%', class: 'card-img' }, []));

    if (card.stamp) {
      childs.push(builder.element('img', { class: 'selo-novidade', src: path + 'stamp-' + card.stamp + '.png', draggable: "false", height: '20%', width: '25%' }, []));
    }
...
function lista(lista, game) {
  ...
  function addCard(card, index, array) {
    ...
    let imgName = card.class.toLowerCase().replace(' ', '') + '/' + card.cardnumber + '.png';

    childs.push(builder.element('img', { src: path + game + '/cards/' + imgName, draggable: "false", height: '100%', width: '100%', class: 'card-img' }, []));
