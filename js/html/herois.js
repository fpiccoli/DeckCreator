...
function build(type, colunas, puros, lista, game) {
  let html = builder.build(classe(type, colunas, puros, lista, game));
...
function classe(type, colunas, classes, lista, game) {
  let json = [];
  ...
  for (let i in classes) {
    ...
    if (type == 'pure') {
      child = content(classes[i], 'selecionar-' + stringManager.getNome(classes[i].name), type, game);
      cor = 'panel panel-default'
    }
    else if (hibridos.length > 0) {
      child = hibrido(hibridos, classes[i].name, type, game);
      cor = 'panel panel-default'
    }
...
function hibrido(hibridos, classe, type, game) {
  let jsonNav = [];
  let jsonContent = [];
  ...
  for (let i in hibridos) {
    ...
    let heroi = content(hibridos[i], 'selecionar-' + stringManager.getNome(classe) + '-' + stringManager.getNome(sub2), type, game);
...
function content(classe, selecionarHeroi, type, game) {
  let json = [];
