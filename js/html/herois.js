...
function classe(type, colunas, classes, lista){
  ...
  for (let i in classes){
    ...
    if (type == 'pure'){
      child = content(classes[i], 'selecionar-'+classes[i].name.toLowerCase());
    ...
    let collapse = builder.element('div', {id: type+'-'+classes[i].name.toLowerCase(), class: 'panel-collapse collapse',  ariaExpanded:'false'}, [body])
...
function hibrido(hibridos, classe){
  ...
  for (let i in hibridos){
    ...
    let link = builder.element('a', {href: '#'+classe.toLowerCase()+'-'+hibridos[i].name.toLowerCase().replace(' ','')+'-nav', dataToggle: 'tab'}, [text]);
    jsonNav.push(builder.element('li', {class: 'text-center '+ativo+sub}, [link]));

    let heroi = content(hibridos[i], 'selecionar-'+classe.toLowerCase()+'-'+sub2.toLowerCase());
    jsonContent.push(builder.element('div', { class: ativo2, id: classe.toLowerCase()+'-'+hibridos[i].name.toLowerCase().replace(' ','')+'-nav' }, heroi));
  }
